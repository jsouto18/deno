...
fn op_connect(
	...
	assert!(args.transport == "tcp" || args.transport == "unix"); // TODO Support others.
	let state_ = state.clone();
	if args.transport == "tcp" {
		...
		return Ok(JsonOp::Async(op.boxed_local()));
	}
...
fn op_listen(
	...
	if args.transport == "uds" {
		let address_path = Path::new(&args.address);
		state.check_read(&address_path)?;
		let (rid, local_addr) = listen_uds(state, &address_path)?;
		debug!(
			"New listener {} {}",
			rid,
